...
#include <Adapter/AdapterComponentAc.hpp>
...
namespace OBC_project {
  ...
  Fw::InputCmdPort *AdapterComponentBase ::
    ...
    regCommands()
  {
    ...
    this->m_cmdRegOut_OutputPort[0].invoke(
        this->getIdBase() + OPCODE_TRANSFER_PACKET
    );
  }
    ...
    )
  {
    ...
    switch (opCode - idBase) {
      ...
      case OPCODE_TRANSFER_PACKET: /*!< Transfer a data packet */
        compPtr->TRANSFER_PACKET_cmdHandlerBase(
            opCode,
            cmdSeq,
            args
        );
  ...
  void AdapterComponentBase ::
    TRANSFER_PACKET_cmdHandlerBase(
        FwOpcodeType opCode,
        U32 cmdSeq,
        Fw::CmdArgBuffer &args
    )
  {
    ...
    this->TRANSFER_PACKET_preMsgHook(opCode,cmdSeq);
    ...
    // Serialize for IPC
    _status = msg.serialize(static_cast<NATIVE_INT_TYPE>(CMD_TRANSFER_PACKET));
    FW_ASSERT (
    ...
    )
  {
    ...
    if (this->m_logTextOut_OutputPort[0].isConnected()) {
      ...
      const char* _formatString =
        "(%s) %s: Data packet received: %s";
#else
      const char* _formatString =
        "%s: Data packet received: %s";
#endif
    ...
    doDispatch()
  {
    ...
    switch (msgType) {
      ...
      // Handle command TRANSFER_PACKET
      case CMD_TRANSFER_PACKET: {
        // Deserialize opcode
        ...
        this->TRANSFER_PACKET_cmdHandler(
            opCode,
            cmdSeq,
            packet
        );
